...
import Data.Aeson (Result(..), Value, fromJSON, toJSON, ToJSON, FromJSON)
import Data.Time
...
postgresResponseLogger :: (ToJSON a) => TableName -> IO Connection -> ConversationId -> a -> IO ()
...
makePostgresLogger :: (ToJSON a) => TableName -> IO Connection -> ConversationId -> a -> IO ()
makePostgresLogger = postgresResponseLogger

createTableQuery :: TableName -> Query
createTableQuery tableName = fromString $ toString $ 
...
insertQuery tableName = fromString $ toString $ 
    "INSERT INTO " <> tableName <> " (conversation_id, response, logged_at) VALUES (?, ?, ?)"

selectAllQuery :: TableName -> Query
selectAllQuery tableName = fromString $ toString $ 
    "SELECT id, conversation_id, response, logged_at FROM " <> tableName <> " ORDER BY logged_at DESC"

selectByTimeRangeQuery :: TableName -> Query
selectByTimeRangeQuery tableName = fromString $ toString $ 
    "SELECT id, conversation_id, response, logged_at FROM " <> tableName <> 
...
getLogsByTimeRange :: (FromJSON a, Typeable a) => TableName -> IO Connection -> UTCTime -> UTCTime -> IO [LogEntry a]
